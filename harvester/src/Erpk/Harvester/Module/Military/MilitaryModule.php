    ...
    public function getActiveCampaigns()
    {
        ...
        foreach ($ul as $type => $xpath) {
            ...
            foreach ($campaigns as $li) {
				...
				$dat = array();
				$dat['rw'] = $li->select('img[@class="resistance_sign one"]')->hasResults();
				$dat['influence'] = $li->select('span[@class="goright influence_multiplier"]')->hasResults()?$li->select('span[@class="goright influence_multiplier"]')->extract():"";
				
				$copos = 'strong/q';
				$co = $li->select($copos)->hasResults()?$li->select($copos)->extract():"";
				$dat['atacker'] = str_ireplace($co,"",$li->select('strong')->extract());
				$dat['atacker_points'] = str_ireplace($co,"",$li->select('q[@class="scorePoints score_left"]')->extract());
					
				$copos = 'div[@class="opponent_holder"]strong/q';
				$co = $li->select($copos)->hasResults()?$li->select($copos)->extract():"";
				$dat['defender'] = str_ireplace($co,"",$li->select('div[@class="opponent_holder"]/strong')->extract());
				$dat['defender_points'] = str_ireplace($co,"",$li->select('q[@class="scorePoints score_right"]')->extract());
				$dat['region'] = $li->select('a[@class="county"]')->extract();
				$dat['time'] = $li->select('span[@class="countyTime"]')->extract();
